...
// Discord-specific directives prompt (loaded once per process)
...
async function handleMessageCreate(token: string, message: DiscordMessage): Promise<void> {
  ...
  try {
    ...
    const discordDirectives = await loadDiscordDirectives();
    if (discordDirectives) promptParts.push(discordDirectives);
    if (skillContext) {
