...
// Telegram-specific directives prompt (loaded once per process)
...
async function handleMessage(message: TelegramMessage): Promise<void> {
  ...
  // Without this, every Telegram chat funnels into the global session.
  ...
  try {
    ...
    const telegramDirectives = await loadTelegramDirectives();
    if (telegramDirectives) promptParts.push(telegramDirectives);
    if (skillContext) {
